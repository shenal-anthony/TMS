...
const StatusCard = ({ title, value, icon }) => {
  ...
  return (
    <Card sx={{ 
      height: '100%',
      p: isSmallScreen ? 1 : 2, // Reduce padding on small screens
    }}>
      <CardContent sx={{ 
        display: 'flex', 
        alignItems: 'center',
        height: '100%',
        p: isSmallScreen ? '8px !important' : '16px !important', // Adjust padding
      }}>
        <Box sx={{ 
          mr: isSmallScreen ? 1 : 2, // Reduce margin on small screens
          color: 'primary.main',
